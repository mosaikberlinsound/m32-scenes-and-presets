# m32-scenes-and-presets

The Mosaik Berlin's sound board configuration files (i.e., Midas M32) are version controlled in this repo.

The following documentation of the Midas M32 scene file is derived from the documentation of the [Behringer X32 Scene File of Saddleback Berlin Church's Production Team](https://github.com/cabcookie/saddleback-x32-general-scene/blob/master/README.md).

![The Midas M32](/assets/M32-xlarge.jpg)

This description explains the structure of the M32 scene file from top to bottom. Little by little we will be able to describe all parts of the file. At the moment, those parts that have not yet been fully evaluated are marked with the text “Not yet evaluated”.
...
These settings describe how the user controls on the right side of the M32 are configured. This allows very flexible, fast settings to be made, which are otherwise distributed in various submenus.

![M32 User Controls](/assets/user-controls.png)
...
![M32 Channel Settings](/assets/channel-settings.png)
...
![M32 Preamps](/assets/preamp-blocks.png)

```txt
/headamp/000 +41.0 OFF
```

The way we use the soundboard, the amps are mapped on the following channels:
